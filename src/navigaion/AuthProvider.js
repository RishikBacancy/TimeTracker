						...
						return auth().signInWithCredential(googleCredential).then((data) => {
							...
							firestore().collection('Users').doc(auth().currentUser.uid).get().then((snap) => {
								if (snap.exists) {
									...
									userData.name = encryptionData(userId, data.user.displayName);
									userData.email = encryptionData(userId, data.user.email.toLowerCase());
									userData.phone = encryptionData(userId, data.user.phoneNumber);
									userData.image = null;
